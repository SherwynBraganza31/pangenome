...
import numpy as np
import os
from genome_curation_module.busco_handler import BuscoHandler
import shutil
...
class ASMStatParser:
    ...
    def extract_checkM_stats(self):
        """
            ...
            :return:
        """
        dataframe = pd.read_csv(self.working_dir + 'data_summary.tsv', sep='\t')
        ...
        dataframe.to_csv(self.working_dir + 'modded_data_summary.csv')
    ...
    def curate_genomes(self, busco_cutoff: float = 95, checkM_cutoff: float = 95):
    ...
    def create_curated_dir(self):
        ...
        curated_list = self.curated_genome_frame.loc[:, 'Assembly Accession'].to_list()
        for x in os.listdir(self.working_dir):
            if (x in curated_list or x == "curated_genome_frame.csv" or x == "assembly_data_report.jsonl"
                ...
                if os.path.isdir(self.working_dir + x):
                    shutil.copytree(self.working_dir + x, curated_dir_subpath + x)
                else:
                    shutil.copyfile(self.working_dir + x, curated_dir_subpath + x)

        print(f'{len(curated_list)} genomes remain')
    ...
    def plot_genome_stats(self):
        ...
        curated_genome_frame = pd.read_csv(self.source_dir + '/ncbi_dataset/data/curated_genome_frame.csv')
