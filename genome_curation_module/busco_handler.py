...
class BuscoHandler:
                        ...
                        for json_file in os.listdir(f'BUSCO/{x}'):
                            if '.json' in json_file:
                                with open(f'BUSCO/{x}/{json_file}', encoding="utf-8") as f:
                                    ...
                                    self.score_dict.update({x:json_dict['results']})
    ...
    def run_BUSCO_mp(self):
        ...
        def busco_subprocess(fasta_file_list:list):
            for fasta_file in fasta_file_list:
                subprocess.run(f'busco -i {fasta_file} --mode genome --cpu {1}' +
                               f' --lineage_dataset {lineage} -o {self.working_dir}BUSCO/{x}/ -f',
                               shell=False)
                for json_file in os.listdir(f'BUSCO/{x}'):
                    if '.json' in json_file:
                        with open(f'BUSCO/{x}/{json_file}', encoding="utf-8") as f:
        ...
        for x in os.listdir(self.working_dir):  # parse through all assembly folders and files in dir
            if os.path.isdir(self.working_dir + x):  # filter to only get assembly directories
                for fasta_file in os.listdir(self.working_dir + x):
                    if '.fna' in fasta_file:  # only pick the fasta file in assembly dir
                        # run BUSCO analysis
                        busco_fasta_list.append(self.working_dir + x + '/' + fasta_file)

        # print(busco_fasta_list)
        cpus = mp.cpu_count()
        input(f"Found {cpus} CPU Cores. Hit Enter to continue...")
        div = len(busco_fasta_list) // cpus
        with mp.Pool() as p:
            return_vals = p.map(busco_subprocess,
                                list(busco_fasta_list[x:x+div] for x in range(0, len(busco_fasta_list), div)))
    ...
    def update_BUSCO_scores(self):
        ...
        for key in self.score_dict.keys():
            df[df[:, columns.index('Assembly Accession')] == key, -1] = self.score_dict[key]['Complete percentage']
        ...
        df.to_csv(f'{self.working_dir}/data_summary.tsv', sep='\t')
        return
