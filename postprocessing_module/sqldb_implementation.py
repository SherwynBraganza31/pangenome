...
def condense_to_db(dbname: str, path: str, trimmed_matrix: pd.DataFrame, gene_ont: pd.DataFrame):
    ...
    cnx = sqlite3.connect(":memory")  # create and in memory connection

    # Genes Table - Links genes from each organism to Gene Families
    genes_table = trimmed_matrix.loc[:, ['Gene ID', 'Gene Family']].set_index('Gene ID')
    genes_table.to_sql('genes', cnx, if_exists='fail')

    # Organisms Table - Links organism name to Gene Family and Genome ID
    ...
    # Gene Families Table - Links Gene Families to a bunch of parameters
    ...
    # GO Table - Links GO IDs to a other common params
    ...
    # Uniprot Table - Links UniProts to the protein it signifies and the # of times its found/
    # the number of times the gene is found.
