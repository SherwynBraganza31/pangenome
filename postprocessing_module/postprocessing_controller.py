...
from postprocessing_module.sqldb_implementation import condense_to_db
import json
import pandas as pd
...
class PostprocessingController:
    def __init__(self, source_dir):
        ...
        self.postproc_dir = self.source_dir + 'postprocessing/'
        self.grabStatReport()
        self.callUniProtExtractor()
        self.callMatrixTrimmer()
        self.callUniProtREST()
        self.callGeneOntProc()
        condense_to_db('genes.db', self.postproc_dir,
                       trimmed_matrix=pd.read_csv(self.postproc_dir+'genes.csv'),
                       gene_ont=pd.read_csv(self.postproc_dir+'go_ids.csv'))
        return
    ...
    def callUniProtREST(self):
        genes = pd.read_csv(self.source_dir + 'postprocessing_results/genes.csv')
        with open(self.source_dir + 'ppanggolin_run/partitions/partitions/exact_core.txt', 'r') as file:
            exact_core = file.read()
            exact_core = exact_core.split('\n')

        gene_families = genes[genes.loc[:, 'Gene ID'].isin(exact_core)]
        uniprots = gene_families.loc[:, 'UniProtKB'].to_numpy().astype(str)
        ...
        with open(self.source_dir + "postprocessing_results/go_results.json", 'w', encoding='utf8') as ofile:
