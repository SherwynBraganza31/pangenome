...
class UniProtExtractor:
    def __init__(self, source_dir):
        ...
        self.ppan_dir = source_dir + 'ppanggolin_run/'
    ...
    def parseGFF(self):
        ...
        for file in file_list:
            ...
            curr_file = gffutils.create_db(self.gff_path + file, dbfn=':memory:',
    ...
    def parseGFF_mp(self):
        ...
        def mprocess_gff_file(files: list):
            it_gene_count = {}
            it_out_df = pd.DataFrame(columns=['Gene ID', 'Gene Symbol', 'UniProtKB',
                                              'Annotation', 'Organism'])
            for file in files:
                ...
                for row in curr_file.execute('SELECT attributes FROM features'):
                    # parse string attributes into dict
                    attribs = json.loads(row['attributes'])
                    res = updateDatabase(it_out_df, attribs, file.split('.')[0])
                    count += 1
                    pbar.update(1)
                it_gene_count.update({file: count})

            return it_gene_count, it_out_df

        cpus = mp.cpu_count()
        input(f"Found {cpus} CPU Cores. Hit Enter to continue...")
        div = len(file_list)//cpus
        with mp.Pool() as p:
            return_vals = p.map(mprocess_gff_file, list(file_list[x:x+div] for x in range(0, len(file_list), div)))

        out_df = pd.concat(return_vals[x][1] for x in range(0, len(return_vals)))
        for x in range(len(return_vals)):
            gene_count.update(return_vals[x][0])

        uniprot_freqs = np.unique(out_df.loc[~out_df['UniProtKB'].isna(), ['UniProtKB']].to_numpy(),
                                  return_counts=True)
        uniprot_freqs = dict(zip(uniprot_freqs[0].astype(str), uniprot_freqs[1].astype(str)))
        with open(self.results_path + 'uniprot_freqs.json', 'w') as ofile:
            json.dump(uniprot_freqs, ofile)
