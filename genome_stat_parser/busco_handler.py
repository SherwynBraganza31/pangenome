...
class BuscoHandler:
    ...
    """
    def __init__(self, source_dir: str, busco_dir: str = None):
        """
        ...
        self.busco_dir = self.source_dir + '/' if busco_dir is None else busco_dir
        self.score_dict = {}
    ...
    def run_BUSCO(self):
        ...
        for x in os.listdir(self.source_dir):  # parse through all assembly folders and files in dir
            if os.path.isdir(self.source_dir + x):  # filter to only get assembly directories
                for fasta_file in os.listdir(self.source_dir + x):
                    if '.fna' in fasta_file:  # only pick the fasta file in assembly dir
                        # run BUSCO analysis
                        subprocess.run(f'busco -i {self.source_dir}{x}/{fasta_file} --mode genome --cpu {cpus}' +
        ...
        with open(self.source_dir + 'busco_scores.json', 'w') as file:
    ...
    def update_BUSCO_scores(self):
        ...
        :return: None
        """
        df = pd.read_csv(f'{self.source_dir}/data_summary.tsv', sep='\t')
        ...
        if not bool(self.score_dict):  # if dictionary is empty
            try:
                with open(self.source_dir + 'busco_scores.json', 'r') as file:
        ...
        df.to_csv(f'{self.source_dir}/modded_data_sumamry.csv')
        return


if __name__ == '__main__':
    busco_handler = BuscoHandler(source_dir='/home/sbraganza/projects/171genomes/171genomes_ncbi/ncbi_dataset/data')
