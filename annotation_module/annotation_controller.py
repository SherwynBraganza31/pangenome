...
class AnnotationController:
    def __init__(self, source_dir):
        ...
        if not check_source_dir(source_dir):
            print('Ending program ......')
            return

        self.prepped_fasta_dir = self.source_dir + "prepped_fasta/"
        self.assembly_path = self.source_dir + "curated_genomes/data/"
...
def check_source_dir(dir):
    ...
    if "curated_genomes" not in os.listdir(dir):
        print('Source Directory does is missing the curated genomes directory')
        return None
    else:
        return dir
